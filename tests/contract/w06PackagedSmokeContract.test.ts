...
describe("W06 packaged Windows smoke contract", () => {
  it("retains exact display annotations and the packaged internal-click target", async () => {
    ...
    try {
      ...
      const destination = supported[1]?.dest;
      if (!Array.isArray(destination)) throw new Error("Second supported annotation must be an internal destination");
      const pageIndex = typeof destination[0] === "number" ? destination[0] : await pdf.getPageIndex(destination[0]);
  ...
  it("keeps W08 as a fixture-bound ordinary link navigation scenario", async () => {
    ...
    expect(script).toContain("PDF link 2");
    expect(script).toContain("[System.Windows.Automation.InvokePattern]$activation).Invoke()");
    expect(script).toContain("W08 internal link did not navigate to page two");
    expect(script).toContain("w08:ordinary-link-navigation");
    expect(script).toContain("W08 failed to restore page one");
    expect(script).not.toContain("w08:hints-");
  ...
  it("awaits tab activity publication and never swallows revocation failure", async () => {
    ...
    expect(main).toMatch(/await session\.navigateToDestination\(page, destination, cause, isActivationCurrent, returnLanding\)/);
