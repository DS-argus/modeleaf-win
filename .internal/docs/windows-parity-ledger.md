...
| Search | `ReaderSearchWorkflowTests`, including per-tab isolation | CP3–CP4 | CP3 ships bounded trimmed case-insensitive literal search; CP4 retains reader and search state per owning tab. |
...
## CP4 implementation record

- Palette and help enabled state derive canonically from the typed action registry; dispatch does not advertise a no-op action.
- The shell owns eight ordered tabs. Each tab retains tab-local reader, view, and search state; inactive heavy resources are evicted under the global limits.
- Recent-file state is native-only and capped at 15 entries. The renderer receives only opaque recent IDs and basenames, never persisted source paths.
- Native chooser, file drop, startup, CLI, second-instance delivery, and manually browsed Open With argv converge on an owner-checked, exactly-once native request coordinator.
- The process is bounded to four native window owners and eight sessions globally; the two-owner workspace/session-model harness proves aggregate eight-session accounting. This is not a product second-window UI claim. No renderer open paths are introduced, and installed file association is not claimed.
- Verification passes 228 Vitest cases, 86 Rust tests, production web build, the no-bundle release build, and current headless registry/tab behavior. The retained executable SHA-256 is `f90e1657364b261e7228211689b5313a1f2fad760e8cf5203d93d9502863c2c6`.
- Source-bound native lifecycle evidence for `.git/cp4-generation30.diff` (`4965b5b1524d1c1288a3104b15fd9a9d790fd7654d5812d2f555cf7094a5383f`) proves 20/20 clean launches and closes, exact per-launch Job Object cleanup, zero process remnants, same-path second-instance/direct-argv/drop idempotency, and no tab admission for remote argv. WebView2 UIA exposed tab selection/count but not PDF canvas/status/name semantics; those remain covered by the production-browser and source tests. The direct-argv leg is explicitly not evidence of the actual Explorer “Open with” picker, which remains an outstanding manual route gate.
