...
- `scroll.*`와 `page.*`의 제외는 menu presentation에만 적용한다. Issue #79 최종 사용자 결정은 `history.*`, `config.*`, `update.*` 여섯 항목을 공통 메뉴·palette·help 목록에서 숨긴다. action ID, registry category, canonical binding과 실제 dispatch는 유지하며 다른 menu group으로 재배치하지 않는다. 공통 shortcut formatter는 영문 대문자 literal을 `Shift+소문자`, modifier chord의 영문 base를 소문자로 표시하되 실제 Shift modifier를 임의로 추가하지 않는다.
