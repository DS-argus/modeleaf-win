...
- 두 모드는 동시에 표시할 수 있다. 상태 문구나 배율에서 모드를 추정하지 않으며 추가 `FIT WIDTH` 배지는 없다.
...
- 검증 범위와 미완료 native/Narrator 항목은 [`issue78-status-badges.json`](../evidence/issue78-status-badges.json)을 따른다. Browser DPR/text-size/forced-colors emulation은 Windows 설정/Narrator 검증을 대신하지 않는다.
