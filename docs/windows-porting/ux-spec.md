...
- Issue #71 candidate: initial and additional WebViews disable Chromium `HideCursorWhileTyping` so typing does not hide Modeleaf's pointer before a native-dialog transition. This does not change Windows pointer settings or force cursor/focus state. Physical packaged cursor acceptance remains pending; see testing-risks §13.
