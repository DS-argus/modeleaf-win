...
Issue #53 owner delta: 이 문서의 TOC와 keyboard-hint 원본 링크는 재개발 reference이며 현재 Windows runtime 요구가 아니다. Windows는 일반 링크 클릭을 유지하고 TOC 3개/hint 1개 action을 제외해 50 actions를 사용한다. Windows 구현의 보존 지점은 `deferred-toc-link-hints.md`에 기록한다.
