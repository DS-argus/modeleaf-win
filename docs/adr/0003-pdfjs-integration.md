...
The retained inline progress/cancel control coexists with existing status diagnostics. It reports opening, monotonically updated page preparation, `Submitted to printer`, cancellation, and failure; per-page submission acknowledgements do not replace the preparation message. Submitted means the native job was submitted, not that physical output succeeded. Plain `Escape` cancels only while the control owns focus; modified keys and IME/composition input are not intercepted.
