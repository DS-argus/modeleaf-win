...
| 8. Embedded-outline TOC | W06, W09, W10 | `not-started` | `feature-spec.md` §8; `ReaderOutline.swift`; `TOCWidgetView.swift`; PR #45 | No dedicated outline/TOC model or TOC widget is present in `src/`. | `intentional-delta`: the TOC is owned by the active tab/window, not a pane (ADR 0001). | W10 real outline adapter, normalization/selectors, active-tab widget, and 399/400ms fake-clock gate. |
| 9. Tabs and window duplication | W03, W05, W09 | `partial` | `feature-spec.md` §9; `TabStore.swift`; `PaneCoordinator.swift`; `ReaderDuplicationSnapshot.swift` | `src/core/TabWorkspace.ts` and `main.ts` implement a window-scoped tab workspace. | `intentional-delta`: panes, split topology, and split duplication are removed by owner directive (ADR 0001). Independent same-process windows replace side-by-side panes. | W09 window-scoped tab ownership, same-process two-window isolation, and Explorer/second-instance routing gates. |
...
| W03 | `partial` | Current core modules cover portions of actions, bindings, state, workspace, and themes. | Pure TS contracts must exactly cover all 54 actions, input/config/history/outline/theme/update reducers. |
...
| W09 | `partial` | Tab workspace and native open-request code exist; window-scoped tab ownership and multiwindow parity are incomplete. | Window-scoped tabs, same-process two-window, and Explorer routing gates. |
| W10 | `not-started` | No dedicated embedded-outline projection or TOC UI evidence exists. | Complete active-tab TOC contract and fixture suite. |
