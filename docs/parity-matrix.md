...
**Last audited:** 2026-08-18 against `origin/main` @ `dd1b14a`, PRs #3–#25 merged.
...
| 8. Embedded-outline TOC | W06, W09, W10 | `partial` | `feature-spec.md` §8; `ReaderOutline.swift`; `TOCWidgetView.swift`; PR #45 | Pure domain only: `src/domain/outlines/OutlineModel.ts` and `OutlineSelector.ts` with 14 tests including `tests/integration/pdfOutlineProbe.test.ts`. **No TOC widget exists and `toc.toggle`, `toc.scrollDown`, and `toc.scrollUp` are unwired in `src/main.ts`.** | `intentional-delta`: the TOC is owned by the active tab/window, not a pane (ADR 0001). | W10 outline adapter, active-tab widget, history integration, and the 399/400ms fake-clock gate. |
...
| W10 | `not-started` | Pure outline domain exists from W03 (`OutlineModel.ts`, `OutlineSelector.ts`, 14 tests), but no TOC widget, no adapter wiring, and three unwired actions. | Complete active-tab TOC contract, widget, history integration, and fixture suite. |
