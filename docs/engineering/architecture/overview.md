...
Passive Fit Width page selection updates the active/visible pages without choosing another scale from the newly detected page; explicit fit, navigation, rotation and viewport resize remain presentation owners. This prevents mixed-size boundaries from feeding their own layout changes back into passive fitting. Fit Page checks the final client geometry after single-page publication and corrects a scrollbar-induced size change under the same presentation lease, without delaying unchanged-geometry input.
