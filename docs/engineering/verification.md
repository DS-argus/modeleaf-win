...
The same runner includes a fit-geometry matrix on generated mixed-size and same-size PDFs at 800×600 CSS pixels, DPR 1/1.25/1.5, and normal/120ms-delayed real PDF.js render-task completion. It checks passive boundary scale stability, repeated `w`/`F`, final padded client-box sizing, zoom/rotation/resize, and evicted tab reactivation. Set `READER_QA_FIT_ONLY=all`, `fit-width`, or `fit-page` to isolate these cases; omit it for the complete matrix. Failure evidence records the first divergent geometry and finite ownership/settlement samples. These are renderer/headless checks, not evidence that the affected internal environment passed.
