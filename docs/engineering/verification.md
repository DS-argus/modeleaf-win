...
Admission-evidence checks (exclusive native ownership required): `cargo test --locked --manifest-path src-tauri/Cargo.toml --lib pdf_session::tests::admission_`, `--lib native_io::tests`, `--lib pdf_protocol::tests`, and `--test cp5_diagnostics`. The controlled same-file test runs real native protocol workers behind a held local file mutex: four permits/queued operations, zero promoted operations, then a fifth503 with `OUTER_PROTOCOL_RANGE_GATE`. Caller abandonment/close must not free permits early; release/join and handle cleanup must complete before capacity reuse. Isolated manager tests fill each actual admission predicate and assert the original tag, captured occupancy, released locks and accounting recovery. These controlled local tests are not proof that the owner's private file naturally produces that request pattern or that the same rejector caused its failure. A renderer503 alone is not a NAS response, file-size rejection or native-stage proof.
