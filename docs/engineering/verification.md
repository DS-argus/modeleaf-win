...
Use only an approved same-user resource and an identified `preview:worktree` candidate for mapped-drive versus equivalent UNC, successful-folder and local-copy controls. Test one anonymized case at a time; retain source hashes locally. Collect only finite stage, tag, outcome and optional numeric OS code from the native local diagnostic log. Do not share paths, account/server/share names, document names/contents, capability tokens or credentials. Record timeout/cancellation and physical settlement separately; the diagnostic queue can drop evidence, and a missing event does not identify a phase or prove success. Policy rejection, worker admission and transport-only failures are not instrumented by this native file-I/O slice. A root cause or network correction requires actual permitted case evidence, not a successful mock or local fixture.
