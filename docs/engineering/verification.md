...
New normal-version publications (including v0.3.0) use stable GitHub classification and Latest; SemVer prerelease tags remain prereleases and are not Latest. Classification does not certify unperformed native, installed, signing, or private-network checks. An already-published release is verified without changing its assets, notes, classification, or Latest status, including historical experimental releases with normal-version tags.
