import { isPdfFailureDiagnostic, type PdfFailureDiagnostic, type PdfDiagnosticReceipt } from "../core/PdfFailureDiagnostic";
...
export function decodePdfDiagnosticReceipt(value: unknown): PdfDiagnosticReceipt {
  ...
  if (Object.keys(item).length !== 4 || Object.keys(item).some((key) => !["delivery", "worker", "dropped", "writeFailures"].includes(key)) ||
    ...
    !["RUNNING", "STOPPED", "UNAVAILABLE"].includes(item.worker as string) || !bounded(item.dropped) || !bounded(item.writeFailures)) throw new Error("PDF_DIAGNOSTIC_CONTRACT");
...
export function createPdfFailureReporter(invoke: NativeInvoke): PdfFailureReporter {
  ...
  return async (failure) => {
    ...
    if (pending >= MAX_PENDING_REPORTS) throw new Error("PDF_DIAGNOSTIC_BUSY");
    pending += 1;
    try {
