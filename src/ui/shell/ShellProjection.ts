...
export function projectWindowShell(snapshot: WindowShellSnapshot): ShellProjectionResult {
  ...
  const tab = snapshot.activeTabId === undefined ? undefined : snapshot.tabs.find(({ id }) => id === snapshot.activeTabId);
  if (snapshot.activeTabId !== undefined && tab === undefined) return { ok: false, code: "ACTIVE_TAB_MISSING" };
